...
params:
  ...
  version: "1.0"
  analysis_dirname: "01-network-comparison"
...
```{r child='child/01-filtered-consensus-net-comparison.Rmd'}
```

Results
=======

```{r child='child/01-filtered-consensus-net-results.Rmd'}
```

```{r save_rdata}
# save RData files to output dir
save(dat, dat_num_unique,  dat_ratio_enrichment, adjmat,
...
html_filepath <- file.path(output_prefix, paste0('network_comparison_', MANUSCRIPT_CONFIG$output_dirname, '.html'))

if (opts_knit$get("rmarkdown.pandoc.to") == 'latex') {
    system(sprintf('(sleep 30 && mv 01-network-comparison.pdf %s) &', html_filepath))
} else {
    system(sprintf('(sleep 30 && mv 01-network-comparison.html %s) &', html_filepath))
}
```
