...
id_a = networks$id[1]
load(file.path(MANUSCRIPT_CONFIG$rdata_dir, sprintf("%d.RData", id_a)))
...
for (i in 1:nrow(networks)) {
    ...
    load(file.path(MANUSCRIPT_CONFIG$rdata_dir, file_a))
    ...
    for (j in 1:i) {
        ...
        load(file.path(MANUSCRIPT_CONFIG$rdata_dir, file_b))
...
```{r results='asis'}
cat(sprintf("- Min: %0.2f\n", min(upper.tri(output_jaccard))))
cat(sprintf("- Max: %0.2f\n", max(upper.tri(output_jaccard))))
cat(sprintf("- Median: %0.2f\n", median(upper.tri(output_jaccard))))
```
...
heatmap.2(output_jaccard, trace='none', dendrogram='row', margin=c(12,12),
...
```{r results='asis'}
cat(sprintf("- Min: %0.2f\n", min(upper.tri(output_rand))))
cat(sprintf("- Max: %0.2f\n", max(upper.tri(output_rand))))
cat(sprintf("- Median: %0.2f\n", median(upper.tri(output_rand))))
```

```{r network_module_similarity_adjusted_rand, cache=TRUE, autodep=TRUE, message=FALSE}
rownames(output_rand) = 1:num_networks
heatmap.2(output_rand, trace='none', dendrogram='row', margin=c(12,12),
          RowSideColors=rank_colors, ColSideColors=network_num,
          cexRow=heatmap_cex, cexCol=heatmap_cex, col='plasma',
          symbreaks=FALSE, symkey=FALSE)
```
...
```{r results='asis'}
cat(sprintf("- Min: %0.2f\n", min(upper.tri(output_kappa))))
cat(sprintf("- Max: %0.2f\n", max(upper.tri(output_kappa))))
cat(sprintf("- Median: %0.2f\n", median(upper.tri(output_kappa))))
```

```{r network_module_similarity_weighted_kappa, cache=TRUE, autodep=TRUE, message=FALSE}
rownames(output_kappa) = 1:num_networks
heatmap.2(output_kappa, trace='none', dendrogram='row', margin=c(12,12),
          RowSideColors=rank_colors, ColSideColors=network_num,
          cexRow=heatmap_cex, cexCol=heatmap_cex, col='plasma',
          symbreaks=FALSE, symkey=FALSE)
```
