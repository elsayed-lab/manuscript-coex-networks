...
```{r random_network_similarity, results='asis', message=FALSE, cache=TRUE, autodep=TRUE}
random_output <- matrix(nrow=num_networks, ncol=num_networks)
rownames(random_output) <- colnames(random_output) <- paste0("network_", networks$id)
...
```{r results='asis'}
cat(sprintf("- Min: %0.2f\n", min(upper.tri(random_output))))
cat(sprintf("- Max: %0.2f\n", max(upper.tri(random_output))))
cat(sprintf("- Median: %0.2f\n", median(upper.tri(random_output))))
```

```{r similarity_plot_permuted, cache=TRUE, autodep=TRUE}
heatmap.2(random_output, trace='none', dendrogram='row', margin=c(12,12),
