...
<!-- Correlation between functional and TF regulon enrichment (# unique annot) -->
```{r, function_coreg_correlation_unique_annots, results='asis', fig.height=3}
plts <- list()

if ('unique_marbach_terms' %in% colnames(df)) {
    # GO correlation
    go_tf_cor <- cor(df$unique_go_terms, df$unique_marbach_terms)

    # position for correlation score text
    #xpos <- 0.90 * max(df$unique_go_terms)
    xpos <- min(df$unique_go_terms) + 0.15 * max(df$unique_go_terms)
    ypos <- 0.95 * max(df$unique_marbach_terms)

    cat(sprintf("- GO vs. TF Pearson correlation (# Unique annotations): %0.2f\n", go_tf_cor))

    plts[[1]] <- ggplot(aes(x=unique_go_terms, y=unique_marbach_terms), data=df) + 
        geom_point(size=0.5) + 
        geom_smooth(method='lm') +
        annotate("text", x=xpos, y=ypos, parse=TRUE, color='red',
                 label=sprintf("paste(italic(r), \" = %0.2f\")", go_tf_cor)) +
        coord_cartesian(xlim=c(min(df$unique_go_terms) - 15, max(df$unique_go_terms) + 30), 
                        ylim=c(min(df$unique_marbach_terms) - 15, max(df$unique_marbach_terms) + 15), 
                        expand=FALSE) +
        theme_bw_high_res() +
        xlab("# Unique GO terms") +
        ylab("# Unique TF regulon genes")

    cpdb_tf_cor <- cor(df$unique_cpdb_terms, df$unique_marbach_terms)
    
    #xpos <- 0.90 * max(df$unique_cpdb_terms)
    xpos <- min(df$unique_cpdb_terms) + 0.15 * max(df$unique_cpdb_terms)
    ypos <- 0.95 * max(df$unique_marbach_terms) 

    # CPDB correlation
    cat(sprintf("- CPDB vs. TF Pearson correlation (# Unique annotations): %0.2f\n\n", cpdb_tf_cor))

    plts[[2]] <- ggplot(aes(x=unique_cpdb_terms, y=unique_marbach_terms), data=df) + 
        geom_point(size=0.5) + 
        geom_smooth(method='lm') +
        annotate("text", x=xpos, y=ypos, parse=TRUE, color='red',
                 label=sprintf("paste(italic(r), \" = %0.2f\")", cpdb_tf_cor)) +
        coord_cartesian(xlim=c(min(df$unique_cpdb_terms) - 15, max(df$unique_cpdb_terms) + 30), 
                        ylim=c(min(df$unique_marbach_terms) - 15, max(df$unique_marbach_terms) + 30), 
                        expand=FALSE) +
        theme_bw_high_res() +
