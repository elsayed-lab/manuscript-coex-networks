...
for (i in 1:length(input_nets)) {
    ...
    ratio_nonzero_edges <- c(ratio_nonzero_edges, sum(adjacency_matrix != 0) / length(adjacency_matrix))

    # ratio of edges with values > 0.25, 0.50, 0.75
    ratio_edges_1q <- c(ratio_edges_1q, sum(adjacency_matrix > 0.25) / length(adjacency_matrix))
    ratio_edges_2q <- c(ratio_edges_2q, sum(adjacency_matrix > 0.50) / length(adjacency_matrix))
    ratio_edges_3q <- c(ratio_edges_3q, sum(adjacency_matrix > 0.75) / length(adjacency_matrix))

    # median non-zero edge weight value (global)
    median_edge_weight_global <- c(median_edge_weight_global, median(adjacency_matrix[adjacency_matrix != 0]))

    # median ratio of non-zero edges per gene
    median_ratio_edges_per_gene <- c(median_ratio_edges_per_gene, 
                                     median(apply(tmp, 1, function(x) { sum(x != 0) / nrow(tmp) })))

    median_edge_weight_per_gene <- c(median_edge_weight_per_gene, 
                                     median(apply(tmp, 1, function(x) { median(x[x != 0]) })))
