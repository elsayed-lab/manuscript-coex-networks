...
```{r filtered_consensus_network_setup, results='hide', message=FALSE, cache=TRUE, cache.lazy=FALSE, autodep=TRUE}
...
# cutoffs to compare
#quantile_cutoffs <- seq(0.0, 0.99, by=0.01)
# DEV 2017/12/05
quantile_cutoffs <- c(0.7, 0.8, 0.9) 
...
for (quantile_cutoff in quantile_cutoffs) {
    ...
    if (CONFIG$target == 'host' && CONFIG$host == 'H. sapiens') {
        ...
        num_cpdb <- length(unique(unlist(sapply(module_cpdb_enrichment, function(x) { x$category }))))
        num_unique_cpdb_pathways <- c(num_unique_cpdb_pathways, num_cpdb)
