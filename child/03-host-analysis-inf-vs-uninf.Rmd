...
# set diagonal to zero for all networks
diag(inf) <- 0
diag(uninf) <- 0
diag(diff) <- 0
...
```{r enriched_difference_subnetwork, results='asis', message=FALSE, fig.height=1600/300}
cat('\n')
...
for (module_color in unique(c(enriched_colors_go, enriched_colors_kegg,
    ...
    min_edge_weight <- min(diff[ind, ind])
    max_edge_weight <- max(diff[ind, ind])
    ...
    print(module_profile_plot(counts_long_inf, module_color, line_color='#333333',
                              font_size=18, line_width=0.5, xlabel='Time (hrs)',
                              include_title=FALSE, ylimits=ylims,
                              xangle=MANUSCRIPT_CONFIG$xangle,
                              xhjust=MANUSCRIPT_CONFIG$xhjust,
