...
```{r child='child/02-tf-regulon-enrichment-results.Rmd'}
```
...
```{r child='child/02-summary-of-enrichment-results.Rmd'}
```
...
```{r save_table_results, include=FALSE}
for (table_name in ls()[grepl('^table_[XS]?[0-9]', ls())]) {
    ...
    if (is.matrix(tbl_) || is.data.frame(tbl_)) {
        filename <- sprintf("%s_%s.tab", table_name, MANUSCRIPT_CONFIG$output_dirname)
        ...
        filename <- sprintf("%s_%s.xlsx", table_name, MANUSCRIPT_CONFIG$output_dirname)
...
```{r save_rdata_results, results='asis'}
