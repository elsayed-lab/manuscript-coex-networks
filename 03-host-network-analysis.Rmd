...
params:
  ...
  title: "Host consensus network comparison"
...
counts_inf   <- hstc_inf_count_table
counts_uninf <- hstc_uninf_count_table

mapping_inf <- CONFIG_HSTC$condition_mapping
mapping_uninf <- CONFIG_HSTCUI$condition_mapping

condition_inf   <- factor(CONFIG_HSTC$samples[['Condition']])
condition_uninf <- factor(CONFIG_HSTCUI$samples[['Condition']])

subtitle <- 'Human infected with T. cruzi'

output_suffix <- 'hstc_inf_vs_uninf'
...
```{r save_tables}
table_S1_hstc_inf_vs_uninf <- table_S1
table_S2_hstc_inf_vs_uninf <- table_S2
table_S3_hstc_inf_vs_uninf <- table_S3
```
...
```{r save_tables}
table_S1_hslm_vs_hstc <- table_S1
table_S2_hslm_vs_hstc <- table_S2
table_S3_hslm_vs_hstc <- table_S3

rm(table_S1, table_S2, table_S3)
```
